# Trying to create splash screen animation
...
pg.display.set_caption('Target Shooter: The Game - Logo Animation')
clock = pg.time.Clock()
run = True
...
while run:
    ...
    chpos = ch.rect.topleft

    if chpos[0] < (303-30) and targets == 2:
        ch.move('tr')
    elif chpos[0] >= (303-30) and targets == 2:
        targets = 1
        logo = Logo(targets=1)
        logo.center(size)
        wait = 1

    if chpos[0] < (339-30) and targets == 1:
        ch.move('r')
    elif chpos[0] >= (339-30) and targets == 1:
        targets = 0
        logo = Logo(targets=0)
        logo.center(size)
        wait = 1

    if chpos[0] >= (339-30) and targets == 0 and chpos[0] < (648-30):
        ch.move('dr')

    sprites = pg.sprite.RenderPlain((logo, ch))
    sprites.draw(screen)
